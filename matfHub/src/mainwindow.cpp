      ...
      ui(new Ui::MainWindow)
{
    ...
    connect(ui->pbPlotParse, &QPushButton::clicked, this,
            &MainWindow::plotParse);
...
std::vector<double> cppSplit(const std::string &s)
{
    ...
    while (std::getline(ss, token, ',')) {
        ...
        try {
            ...
            std::cout << token << " " << std::setprecision(15) << value
                      << std::endl;
            niz.push_back(value);
  ...
  // akcije
    ...
    if (noSelected == 1) {
        ...
        connect(renameAction, &QAction::triggered, this, [=]() {
            ...
            QString newName =
                QInputDialog::getText(this, "Preimenuj fajl",
                                      "Nov Naziv:", QLineEdit::Normal, oldName);
        ...
        connect(deleteAction, &QAction::triggered, this, [=]() {
            QModelIndexList selectedIndices = getSelectedIndices(
                view); // meni se indices vise svidja kao mnozina od index nego
                       // indexes ali Bojane slobodno promeni ako zelis nije mi
                       // bitno
    ...
    for (int i = 0; i < ui->fileViewLayout->count(); ++i) {
        ...
        if (QTableView *table = dynamic_cast<QTableView *>(view)) {
            table->horizontalHeader()->resizeSections(
                QHeaderView::ResizeToContents); // TODO iz nekog razloga cini mi
                                                // se da ovo ne radi UVEK ako se
                                                // prvi put ulazi u neki folder?
        }
    }
}
...
void MainWindow::setUpFileView(/*tipPogleda*/)
{
    ...
    connect(ui->detailsFileView, &QTableView::doubleClicked, this,
            [this]() { // TODO koji signal je odgovarajuc? nesto sto odgovara
                       // promeni m_currPath i nista drugo? komplikovan nacin bi
                       // bio currPath retvoriti u klasu koja pored stringa ima
                       // i signal koji se ovde salje pri promeni QStringa ali
                       // to me smara trenutno...
                ui->detailsFileView->horizontalHeader()->resizeSections(
                    QHeaderView::ResizeToContents);
            });
    ...
    for (int i = 0; i < ui->fileViewLayout->count(); ++i) {
        ...
        QAbstractItemView *view = static_cast<QAbstractItemView *>(
            widget); // auto ga vec kastuje u konkretan tip dakle QTableView,
                     // QListView itd B) ali kompajler u daljem kodu to ne zna
                     // nego je u fazonu ok ovo ce sigurno biti bar QObject tako
                     // da ipak mora da se kastuje sto je idiotski ali tako je
                     // D: jednom kada naprave funkcionalne procesore koji lenjo
                     // izracunavaju i kada ceo svet predje na normalne
                     // funkcionalne interpretirane jezike ovo nece biti problem
                     // ;)
        // qDebug() << view;//apdejt: i ovaj staticki kast ga kastuje u klasu
        // koja treba da bude, nagadjam jer je AbstractView apstraktna klasa
        // TODO nauci vise o cppu
        ...
        // connect(ui->detailsFileView, &QTableView::doubleClicked,
        // m_fileManager, &FileManager::fileViewDoubleClicked); ovo ne radi a
        // zelim da proradi, TODO istraziti
...
void MainWindow::reshapeMatrix(unsigned dim1, unsigned dim2, unsigned pos)
{
    ...
    if (scrollAreaWidget->children().isEmpty()) {
        ...
        rows =
            qobject_cast<QVBoxLayout *>(scrollAreaWidget->children().first());
        while (!rows->isEmpty()) {
    ...
    QStringList content = matrixStringToStringList(pos == 1 ? (m1->toString())
                                                            : (m2->toString()));
...
void MainWindow::saveMatrix()
{
    ...
    connect(loadLeft, &QPushButton::clicked, this, [this, index]() {
        ...
        loadMatrix(
            1, matrixStringToStringList(Matrix::getSaved(index)->toString()),
            d1, d2);
    });
    connect(loadRight, &QPushButton::clicked, this, [this, index]() {
        auto [d2, d1] = m3->loadRight(index);
        loadMatrix(
            2, matrixStringToStringList(Matrix::getSaved(index)->toString()),
            d1, d2);
    });
}
...
void MainWindow::plot()
{
    ...
    std::string linspaceString =
        "Plot: (" + lowerBound + ", " + upperBound + ")";
